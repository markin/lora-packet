...
exports.decrypt = function (packet, AppSKey, NwkSKey) {
    ...
    var cipherstream_base64 = CryptoJS.AES.encrypt(CryptoJS.enc.Hex.parse(plain_S.toString('hex')), CryptoJS.enc.Hex.parse(key.toString('hex')), {
...
exports.encryptSessionKey = function (packet, appKey) {
    ...
    var ciphertext = CryptoJS.AES.encrypt(CryptoJS.lib.WordArray.create(packet), CryptoJS.lib.WordArray.create(appKey), {
        mode: CryptoJS.mode.ECB,
        iv: LORA_IV,
        padding: CryptoJS.pad.NoPadding
...
exports.encryptJoinAccept = function (packet, appKey) {
    ...
    var dec = CryptoJS.AES.decrypt(packet.toString('base64'), CryptoJS.enc.Hex.parse(appKey.toString('hex')), {
        mode: CryptoJS.mode.ECB,
        iv: LORA_IV,
        padding: CryptoJS.pad.NoPadding
    });
...
function _metadataBlock_Ai(packet, i) {
    ...
    var Ai_buf = Buffer.concat([new Buffer("0100000000", 'hex'), // as spec
    Dir, // direction ('Dir')
    util.reverse(packet.getBuffers().DevAddr), util.reverse(packet.getBuffers().FCnt), util.bufferFromUInt16LE(0), // upper 2 bytes of FCnt (zeroes)
