...
var constants = {
    ...
    MTYPE_DESCRIPTIONS: ['Join Request', 'Join Accept', 'Unconfirmed Data Up', 'Unconfirmed Data Down', 'Confirmed Data Up', 'Confirmed Data Down', 'RFU', 'Proprietary'],

    MTYPE_DIRECTIONS: [null, null, 'up', 'down', 'up', 'down', null, null],
...
function LoraPacket(packetContents) {
    ...
    function _isPlausible(userFields) {
        if (!util.isDefined(userFields)) {
            return false;
        } else if (util.isDefined(userFields.payload) && util.isDefined(userFields.DevAddr)) {
            return true;
        } else {
