...
let util = require('./util.js')("lora-packet parser");

exports.fromWire = function (contents) {
    let lp = new LoraPacket();
...
function LoraPacket(packetContents) {
    ...
    function _initialiseFromWireformat(contents) {
        ...
        let incoming = new Buffer(contents);

        p.PHYPayload = incoming;

        p.MHDR = incoming.slice(0, 1);

        //        let type = p.MHDR._getMType;
        if (that.isJoinRequestMessage()) {
            ...
            let FCtrl = p.FCtrl.readInt8(0);
            let FOptsLen = FCtrl & 0x0f;
            p.FOpts = p.MACPayload.slice(7, 7 + FOptsLen);
            let FHDR_length = 7 + FOptsLen;
    ...
    function _initialiseFromFields(userFields) {
        let p = {}; // temp packet we use during construction
        that._packet = null;
        ...
        if (util.isDefined(userFields.MType)) {
            if (util.isNumber(userFields.MType)) {
                ...
                let mhdr_idx = constants.MTYPE_DESCRIPTIONS.indexOf(userFields.MType);
                if (mhdr_idx >= 0) {
                    p.MHDR = new Buffer(1);
    ...
    this.getDevEUI = function () {
        return that._packet.DevEUI.toString('hex');
    };

    this.getAppEUI = function () {
        return that._packet.AppEUI.toString('hex');
    };

    this.getDevNonce = function () {
        return that._packet.DevNonce.toString('hex');
    };
    ...
    this.toJson = function () {
        ...
        if (that.isJoinRequestMessage()) {
            ...
            msg.DevAddr = asHexString(p.DevAddr);
            msg.FCtrl = asHexString(p.FCtrl);
            msg.FCnt = asHexString(p.FCnt);
            msg.FOpts = asHexString(p.FOpts);
            msg.mType = that.getMType();
            msg.FOpts = that.getDir();
            msg.FCnt= that.getFCnt();
            msg.ACK = that.getFCtrlACK();
            msg.ADR = that.getFCtrlADR();
        }
        return msg;
    };
}
